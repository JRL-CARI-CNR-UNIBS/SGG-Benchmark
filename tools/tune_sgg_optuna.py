...
Place this file in:
    SGG-Benchmark/tools/tune_sgg_optuna.py
...
def numeric_values(value: Any) -> list[float]:
    ...
    def collect(item: Any) -> None:
        ...
        if isinstance(item, np.ndarray):
            if item.size > 0:
                for x in item.reshape(-1).tolist():
                    collect(x)
            return
...
def numeric_mean(value: Any) -> float:
    ...
    if not numbers:
        raise ValueError(f"Metric contains no finite numeric values: {value!r}")
    return float(np.mean(numbers))


def better(score: float, best_score: float, direction: str) -> bool:
    return score > best_score if direction == "maximize" else score < best_score
...
def suggest_parameter(
    ...
    if kind == "choice":
        ...
        if not values:
            raise ValueError(f"{path}: choice requires non-empty values")
...
def validate_distribution_spec(path: str, spec: Mapping[str, Any]) -> None:
    ...
    if kind == "choice" and not list(spec.get("values", [])):
        raise ValueError(f"{path}: choice requires non-empty values")
    if kind != "choice":
        if "low" not in spec or "high" not in spec:
            raise ValueError(f"{path}: {kind} requires low and high")
        if float(spec["low"]) > float(spec["high"]):
            raise ValueError(f"{path}: low must be <= high")
    if kind in {"quniform", "qrandint"} and float(spec.get("q", 0)) <= 0:
        raise ValueError(f"{path}: {kind} requires q > 0")
    ...
    with open_dict(cfg):
        ...
        if apply_tuning_budget:
            ...
            if max_images is not None and max_images > 0:
                batch_size = max(1, int(cfg.solver.ims_per_batch))
...
def validate_user_configuration(
    *,
    ...
    for path, spec in SEARCH_SPACE.items():
        old = ensure_path_exists(cfg, path)
        validate_distribution_spec(path, spec)
        print(
...
def select_metric_at_k(value: Any, metric_k: int | None) -> float:
    ...
    if isinstance(value, Mapping):
        ...
        normalized: dict[str, Any] = {
            str(k).replace(" ", "").lower(): v for k, v in value.items()
        }
        suffix = str(metric_k)
...
def find_metric_values(result: Any, metric_key: str) -> list[Any]:
    """Find metric_key recursively, supporting direct and dataset-nested output."""
    matches: list[Any] = []
...
def objective_from_results(
    ...
    *,
    ...
    matches = find_metric_values(result, metric_key)
    if not matches:
        available: set[str] = set()

        def collect_keys(item: Any) -> None:
            if isinstance(item, Mapping):
                available.update(str(k) for k in item.keys())
                for child in item.values():
                    collect_keys(child)
    ...
    scores = [select_metric_at_k(value, metric_k) for value in matches]
    score = float(np.mean(scores))
    if not math.isfinite(score):
        raise ValueError(f"Final evaluation score is not finite: {score}")
    return score
...
def enable_inplace_relu(module: torch.nn.Module) -> None:
...
def update_class_counts_from_loader(
    ...
    with open_dict(cfg):
        cfg.model.roi_box_head.num_classes = int(num_obj_classes)
        cfg.model.roi_relation_head.num_classes = int(num_rel_classes)


def as_loader_list(loaders: Any) -> list[Any]:
...
def maybe_load_pretrained_backbone(
    ...
    logger.info(f"Loading pretrained backbone detector from: {path}")
    checkpointer.load_backbone(str(path))
...
def dataset_names_for_split(
    ...
    names = list(datasets_cfg.get(split, []) or [])

    if not names:
        base_name = str(datasets_cfg.get("name", "") or "")
        if base_name:
            names = [f"{base_name}_{split}"]

    if not names:
        names = [f"{split}_{index}" for index in range(count)]

    if len(names) < count:
        names.extend(
            f"{split}_{index}" for index in range(len(names), count)
        )
    return names[:count]
...
def evaluate_checkpoint_fresh(
    *,
    ...
    OmegaConf.save(eval_cfg, evaluation_output_dir / "hydra_config.yaml")
    ...
    update_class_counts_from_loader(eval_cfg, loaders[0])

    model = build_detection_model(eval_cfg)
    device = torch.device(str(eval_cfg.model.device))
    model.to(device)
    ...
    logger.info(f"Post-training evaluation checkpoint: {checkpoint_path}")
    checkpointer.load(str(checkpoint_path))

    backbone_type = str(eval_cfg.model.backbone.get("type", "")).lower()
    if "world" in backbone_type and hasattr(model.backbone, "load_txt_feats"):
        logger.info("Loading text embeddings for YOLO World evaluation")
        stats = get_dataset_statistics(eval_cfg)
        object_classes = stats["obj_classes"][1:]
        model.backbone.load_txt_feats(object_classes)
    ...
    if bool(eval_cfg.model.relation_on):
        iou_types += ("relations",)
    if bool(eval_cfg.model.get("attribute_on", False)):
    ...
    use_amp = str(eval_cfg.dtype).lower() == "float16"
    all_results: Dict[str, Any] = {}

    try:
        for dataset_name, loader in zip(dataset_names, loaders):
            dataset_output = evaluation_output_dir / dataset_name
            output_folder = str(dataset_output) if save_predictions else None
            ...
            with torch.autocast(
                ...
                result = inference(
                    ...
                    iou_types=iou_types,
                    box_only=bool(eval_cfg.model.get("rpn_only", False)),
                    device=str(eval_cfg.model.device),
                    expected_results=eval_cfg.test.get("expected_results", []),
                    ...
                    logger=logger,
                    informative=bool(eval_cfg.test.get("informative", False)),
                )
            synchronize()
...
def objective(
    ...
    *,
    ...
    sampled_overrides = {
        normalize_path(path): suggest_parameter(trial, path, spec)
        for path, spec in SEARCH_SPACE.items()
    }

    trial_dir = experiment_dir / "trials" / f"trial_{trial.number:05d}"
    train_dir = trial_dir / "train"
    eval_dir = trial_dir / f"evaluation_{evaluation_split}"
    ...
    trial.set_user_attr("evaluation_split", evaluation_split)
    trial.set_user_attr("metric_k", metric_k if metric_k else "mean")
    trial.set_user_attr("oom", False)

    with (trial_dir / "sampled_overrides.json").open(
        "w",
        encoding="utf-8",
    ) as file:
        json.dump(sampled_overrides, file, indent=2, default=str)
    ...
    try:
        ...
        set_seed(seed=int(cfg.seed) + int(trial.number))
        ...
        if isinstance(train_loader, (list, tuple)):
            if len(train_loader) != 1:
                raise RuntimeError(
                    f"Expected one training loader, got {len(train_loader)}"
                )
            train_loader = train_loader[0]

        update_class_counts_from_loader(cfg, train_loader)
        OmegaConf.save(cfg, train_dir / "hydra_config.yaml")
        OmegaConf.save(cfg, train_dir / "config.yaml")
        ...
        iters_per_epoch = (
            min(raw_iters, max_iter) if max_iter > 0 else raw_iters
        )
        ...
        for epoch in range(max_epoch):
            logger.info(
                f"Optuna trial {trial.number}: training epoch "
                f"{epoch + 1}/{max_epoch}"
            )
            ...
            # Internal validation is used only to choose the checkpoint and to
            # step schedulers that require a validation metric. The value
            ...
            trial.report(internal_score, step=epoch + 1)

            if better(internal_score, best_internal_score, direction):
                previous_checkpoint = best_checkpoint
                best_internal_score = internal_score
                checkpoint_name = f"best_model_epoch_{epoch}"
                checkpointer.save(
                    checkpoint_name,
                    epoch=epoch,
            ...
            if trial.should_prune():
                ...
                raise optuna.TrialPruned(
                    ...
                    f"internal validation score={internal_score:.6f}"
                )
        ...
        with (trial_dir / "internal_validation_history.json").open(
            "w",
            encoding="utf-8",
        ) as file:
            json.dump(internal_history, file, indent=2)

        trial.set_user_attr(
            "best_internal_validation_score",
            float(best_internal_score),
        )
        trial.set_user_attr("checkpoint", str(best_checkpoint))

        # Release all training objects before rebuilding the model for the
        # independent relation_eval_hydra-style evaluation.
        del model
        model = None
        del optimizer, scheduler, scaler, train_loader, val_loaders, checkpointer
        optimizer = None
        scheduler = None
        scaler = None
        train_loader = None
        ...
        trial.set_user_attr("post_training_evaluation_score", final_score)

        with (trial_dir / "post_training_evaluation.json").open(
            "w",
            encoding="utf-8",
        ) as file:
            json.dump(
        ...
        logger.info(
            f"Optuna objective from fresh checkpoint evaluation: "
            f"{final_score:.6f}"
        )
        return final_score
        ...
        bad_score = -1.0e30 if direction == "maximize" else 1.0e30
        logger.exception("CUDA out of memory; assigning a dominated score")
        clear_cuda()
        return bad_score
...
def save_best_config(
    *,
    ...
    with (experiment_dir / "best_result.json").open(
        "w",
        encoding="utf-8",
    ) as file:
...
def persist_study_tables(study: optuna.Study, experiment_dir: Path) -> None:
    try:
        study.trials_dataframe(
            ...
            multi_index=False,
        ).to_csv(experiment_dir / "all_trials.csv", index=False)
    except Exception as error:
        print(f"Warning: could not save all_trials.csv: {error}")
...
def completed_trial_callback(
    ...
    *,
    ...
    persist_study_tables(study, experiment_dir)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--metric",
        default=None,
        help="Metric alias: mR, R, F1, zR, ng-zR, ng-R or ng-mR.",
    )
    parser.add_argument(
        "--metric-key",
        default=None,
        help="Exact key returned by inference; overrides --metric.",
    )
    parser.add_argument(
        "--metric-k",
        type=int,
    ...
    parser.add_argument(
        ...
        choices=["max", "min"],
        default="max",
        help="Compatibility alias mapped to Optuna maximize/minimize.",
    )
    parser.add_argument(
        "--num-trials",
        "--num-samples",
    ...
    parser.add_argument(
        ...
        type=int,
        default=2000,
        help="Approximate training images per epoch; 0 uses the full loader.",
    )
    parser.add_argument(
        "--evaluation-split",
        choices=["val", "test"],
        default="val",
        help=(
            "Split used by the fresh post-training evaluator. Use val for "
            "hyperparameter selection; reserve test for final reporting."
        ),
    )
    parser.add_argument(
        "--storage-path",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Parallel Optuna workers in this process. Keep 1 for a single "
            "GPU. Defaults to --max-concurrent-trials or 1."
        ),
    )
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Legacy Ray option; used as Optuna n_jobs when --n-jobs is absent.",
    )
    parser.add_argument(
        "--pruner",
        choices=["none", "median"],
        default="none",
        help=(
            "Default none ensures every trial reaches independent final "
            "evaluation. Median pruning uses internal per-epoch validation."
        ),
    )
    parser.add_argument("--pruning-startup-trials", type=int, default=5)
    parser.add_argument("--pruning-warmup-epochs", type=int, default=1)
    parser.add_argument(
        "--save-eval-predictions",
        action="store_true",
        help="Pass output folders to inference during post-training evaluation.",
    )
    parser.add_argument(
        "--keep-nonbest-checkpoints",
        action="store_true",
        help="Keep superseded per-epoch best checkpoints inside every trial.",
    )
    parser.add_argument(
        "--reset-study",
        action="store_true",
        help="Delete the previous SQLite study and trial directory first.",
    )

    # Accepted only so existing Ray command lines do not fail.
    parser.add_argument("--gpus-per-trial", type=float, default=1.0)
    parser.add_argument("--cpus-per-trial", type=float, default=None)
    parser.add_argument("--ray-temp-dir", default=None)
    parser.add_argument("--grace-period", type=int, default=None)
    parser.add_argument("--reduction-factor", type=int, default=None)
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Optional fixed key/value overrides after --opts. Arguments after "
            "--opts are consumed as config overrides."
        ),
    )
...
def main() -> None:
    ...
    if args.num_trials < 1:
        raise ValueError("--num-trials/--num-samples must be >= 1")
    ...
    if args.gpus_per_trial != 1.0:
        raise ValueError(
            "This direct Optuna script currently supports one GPU per trial."
        )
    ...
    if n_jobs != 1:
        raise ValueError(
            ...
            "SGG-Benchmark logging and CUDA device assignment are process-global. "
            "Run separate Optuna worker processes with explicit CUDA_VISIBLE_DEVICES "
            "only after adding a multi-worker storage/device strategy."
        )
    ...
    storage_root = Path(args.storage_path).expanduser().resolve()
    experiment_dir = storage_root / args.experiment_name
    database_path = experiment_dir / "optuna.db"

    if args.reset_study and experiment_dir.exists():
        shutil.rmtree(experiment_dir)

    experiment_dir.mkdir(parents=True, exist_ok=True)
    (experiment_dir / "trials").mkdir(parents=True, exist_ok=True)
    ...
    study.set_user_attr("config_file", config_file)
    study.set_user_attr("task", args.task)
    study.set_user_attr("metric", args.metric)
    study.set_user_attr("metric_key", args.metric_key)
    study.set_user_attr("metric_k", args.metric_k or "mean")
    study.set_user_attr("evaluation_split", args.evaluation_split)

    print("\nOptuna study ready")
    print(f"  study: {study.study_name}")
    print(f"  database: {database_path}")
    print(f"  direction: {direction}")
    print(f"  final evaluation split: {args.evaluation_split}")
    print(f"  objective cutoff: @{args.metric_k}" if args.metric_k else "  objective cutoff: mean")
    print("\nLive dashboard command:")
    print(
        "  uv run optuna-dashboard "
        f"\"sqlite:///{database_path}\" --host 127.0.0.1 --port 8080"
    )

    callback = lambda study_, trial_: completed_trial_callback(
        study_,
        trial_,
        experiment_dir=experiment_dir,
        config_file=config_file,
    ...
    study.optimize(
        lambda trial: objective(
            ...
            metric_k=args.metric_k or None,
            direction=direction,
            max_epochs=args.max_epochs,
            max_images=args.max_images,
            experiment_dir=experiment_dir,
            evaluation_split=args.evaluation_split,
            save_predictions=args.save_eval_predictions,
            keep_nonbest_checkpoints=args.keep_nonbest_checkpoints,
        ),
        n_trials=args.num_trials,
        n_jobs=n_jobs,
        callbacks=[callback],
        gc_after_trial=True,
        show_progress_bar=(n_jobs == 1 and sys.stdout.isatty()),
    )
